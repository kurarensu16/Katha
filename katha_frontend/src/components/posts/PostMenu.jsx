...
const PostMenu = ({ post, onSaveUpdate, onEdit, onDelete, isAuthor }) => {
    const { isLoggedIn, APIService, setView, user } = useAuth();
    const [isSaved, setIsSaved] = useState(post?.is_saved || false);
    const [menuOpen, setMenuOpen] = useState(false);
    const [isSaving, setIsSaving] = useState(false);
    
    // Determine if current user is author if not explicitly provided
    const userIsAuthor = isAuthor !== undefined ? isAuthor : (post && user && post.author_username === user.username);
                            ...
                            {userIsAuthor && onEdit && (
                                <>
                                    <button
                                        ...
                                        className="w-full flex items-center gap-2 text-left px-3 py-2 hover:bg-gray-50 dark:hover:bg-slate-700 text-blue-medium dark:text-blue-accent"
                                    >
